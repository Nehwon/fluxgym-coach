...
class TestImageEnhancer:
    ...
    def test_decode_and_save_base64(self, mock_api_response, tmp_path):
        ...
        enhancer.decode_and_save_base64(mock_api_response["images"][0], output_path)
    ...
    def test_upscale_image(self, temp_image, tmp_path):
        ...
        try:
            ...
            result_path, is_bw = enhancer.upscale_image(
                image_path=temp_image, output_path=output_path, scale_factor=2
            )
...
def test_call_api_connection_error():
    ...
    with patch("fluxgym_coach.image_enhancement.requests.post") as mock_post:
        ...
        enhancer = ImageEnhancer()
        with patch(
            "fluxgym_coach.image_enhancement.time.sleep"
...
def test_call_api_http_404_error():
    ...
    with patch("fluxgym_coach.image_enhancement.requests.post") as mock_post, patch(
        "fluxgym_coach.image_enhancement.time.sleep"
    ) as mock_sleep:
...
def test_call_api_invalid_json():
    ...
    with patch("fluxgym_coach.image_enhancement.requests.post") as mock_post:
        ...
        mock_response.json.side_effect = json.JSONDecodeError(
            "Expecting value", "not a valid json", 0
        )
...
def test_upscale_image_with_invalid_denoising():
    ...
    with tempfile.NamedTemporaryFile(suffix=".png") as temp_file:
        temp_path = Path(temp_file.name)
        with pytest.raises(
            ValueError, match="La force du débruiteur doit être entre 0 et 1"
        ):
...
def test_upscale_image_error_handling(temp_image, tmp_path):
    ...
    with patch("fluxgym_coach.image_enhancement.requests.Session") as mock_session:
        ...
        mock_response.raise_for_status.side_effect = (
            requests.exceptions.RequestException("Erreur de connexion")
        )

        # Configurer le mock de session pour retourner notre réponse
        mock_session.return_value.__enter__.return_value.send.return_value = (
            mock_response
        )
...
def test_upscale_image_with_auto_colorize(temp_bw_image, tmp_path):
    ...
    with patch(
        "fluxgym_coach.image_enhancement.ImageEnhancer.colorize_image"
    ) as mock_colorize, patch(
        ...
        result_path, is_bw = enhancer.upscale_image(
            image_path=temp_bw_image, output_path=output_path, scale_factor=2
        )
...
def test_upscale_image_without_auto_colorize(temp_bw_image, tmp_path):
    ...
    # Créer une image factice pour la réponse de l'API
    mock_response = {"images": [img_str], "parameters": {}}
    ...
    with patch(
        ...
        "fluxgym_coach.image_enhancement.ImageEnhancer.preprocess_image"
    ) as mock_preprocess:
...
def test_upscale_batch_basic(temp_image, tmp_path):
    ...
    try:
        # Appeler la méthode à tester
        results = enhancer.upscale_batch(image_paths=[temp_image], output_dir=tmp_path)
...
def test_upscale_batch_with_bw_images(temp_bw_image, tmp_path):
    ...
    def mock_call_api(endpoint, payload):
        ...
        if "prompt" in payload:
            assert (
                "color" in payload["prompt"].lower()
                or "colorize" in payload["prompt"].lower()
            )
    ...
    try:
        ...
        results = enhancer.upscale_batch(
            image_paths=[temp_bw_image], output_dir=tmp_path
        )
...
def test_upscale_batch_with_custom_output_dir(temp_image, tmp_path):
    ...
    try:
        ...
        results = enhancer.upscale_batch(
            image_paths=[temp_image], output_dir=output_dir
        )
