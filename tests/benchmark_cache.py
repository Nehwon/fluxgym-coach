...
def create_test_image(width: int = 800, height: int = 600, text: str = "") -> Image.Image:
...
def main():
    ...
    parser.add_argument(
        "-n", "--num-images", type=int, default=5,
    ...
    print("\n" + "-"*80)
    print(f"{'CONFIGURATION':<40} | {'MOYENNE':>10} | {'MIN':>8} | {'MAX':>8} | ÉCART-TYPE")
    print("-"*80)
    
    for config, data in stats.items():
        print(f"{config:<40} | {data['avg']:>8.2f}s | {data['min']:>6.2f}s | {data['max']:>6.2f}s | ±{data['std']:.2f}s")
    ...
    if len(stats) >= 2:
        ...
        if first['avg'] > 0 and last['avg'] > 0:
            ...
            print("\n" + "-"*80)
            print(f"GAIN DE PERFORMANCE: {abs(gain):.1f}% {'d''amélioration' if gain > 0 else 'de perte'}")
            print("-"*80)
